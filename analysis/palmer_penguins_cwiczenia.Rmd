...
knitr::include_graphics("data/pingwiny.png", error = FALSE)
```

```{r, out.width = "25%", fig.align ='center'}
knitr::include_graphics("data/dziub.png",error = FALSE)
```
...
```{r, eval=FALSE}
install.packages(c("palmerpenguins", "dplyr", "ggplot2", "kableExtra"))
...
penguins %>%
  count(island) %>%
  kable(col.names = c('Wyspa', 'Liczba')) %>%
...
```{r}
penguins %>%
  summarise(średnia_masa = round(mean(body_mass_g, na.rm = TRUE), 1)) %>%
  kable()
```
...
penguins %>%
  group_by(species) %>%
  summarise(średnia_masa = round(mean(body_mass_g, na.rm = TRUE), 1)) %>%
  kable(col.names = c('Gatunek', 'Średnia masa (g)'))
```
...
ggplot(penguins, aes(x = body_mass_g)) +
  ...
  labs(
    title = "Histogram masy ciała pingwinów",
    x = "Masa ciała (g)",
    y = "Liczba pingwinów"
  ) +
  theme_minimal() +
...
penguins %>%
  ...
  labs(
    title = "Wykres słupkowy gatunków pingwinów",
    x = "Gatunek",
    y = "Liczba pingwinów"
  ) +
  theme_minimal() +
